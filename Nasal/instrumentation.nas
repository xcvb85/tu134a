...
iku_handler = func {
	...
	#Captain panel
	# yellow needle
	var sel_yellow = getprop("Tu-134/instrumentation/iku-1[0]/l-mode");
	...
	if( param_yellow == nil ) param_yellow = 0.0;
	setprop("Tu-134/instrumentation/iku-1[0]/indicated-heading-l", param_yellow );


	# White needle
	var sel_white = getprop("Tu-134/instrumentation/iku-1[0]/r-mode");
	...
	setprop("Tu-134/instrumentation/iku-1[0]/indicated-heading-r", param_white );
}
...
idr_capt_handler = func {
	...
	if(getprop("instrumentation/nav[0]/in-range") == 1 ) {
		...
		{
			setprop("Tu-134/instrumentation/idr-1[0]/caged-flag", 0);
	...
	setprop("Tu-134/instrumentation/idr-1[0]/caged-flag", caged );


	if(distance == nil)
	{
		setprop("Tu-134/instrumentation/idr-1[0]/caged-flag", 1);
		return;
	}

  distance = distance/10.0; # to dec meters, it need for correct work of digit wheels
  setprop("Tu-134/instrumentation/idr-1[0]/indicated-wheels_dec_m", 
  (distance/10.0) - int( distance/100.0 )*10.0 );
  setprop("Tu-134/instrumentation/idr-1[0]/indicated-wheels_hund_m", 
  (distance/100.0) - int( distance/1000.0 )*10.0 );
  setprop("Tu-134/instrumentation/idr-1[0]/indicated-wheels_ths_m", 
  (distance/1000.0) - int( distance/10000.0 )*10.0 );
  setprop("Tu-134/instrumentation/idr-1[0]/indicated-wheels_decths_m", 
  (distance/10000.0) - int( distance/100000.0 )*10.0 );

  distance = distance * 0.621; # km to miles
  setprop("Tu-134/instrumentation/idr-1[0]/indicated-wheels_dec_miles", 
  (distance/10.0) - int( distance/100.0 )*10.0 );
  setprop("Tu-134/instrumentation/idr-1[0]/indicated-wheels_hund_miles", 
  (distance/100.0) - int( distance/1000.0 )*10.0 );
  setprop("Tu-134/instrumentation/idr-1[0]/indicated-wheels_ths_miles", 
  (distance/1000.0) - int( distance/10000.0 )*10.0 );
  setprop("Tu-134/instrumentation/idr-1[0]/indicated-wheels_decths_miles", 
  (distance/10000.0) - int( distance/100000.0 )*10.0 );
}
...
var rv_altitude_update = func {
    ...
    setprop("Tu-134/instrumentation/rv-5m/indicated-altitude-m", alt_m);
}
rv_altitude_update();
...
rv5m_l = func(step) {
  ...
  var ind_alt = getprop("Tu-134/instrumentation/rv-5m/index-m");
  if ( step < 0) {
    if (ind_alt > 99) {
      ind_alt = ind_alt + 10;
    } else {
  ...
  setprop("Tu-134/instrumentation/rv-5m/index-m",ind_alt);
}

setprop("Tu-134/stanchion", 1);
